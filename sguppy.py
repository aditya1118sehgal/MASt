...
def guppy(df):
    ...
    emas = [f for f in list(range(3, 25, 2))]
    print ([f"EMA_{ema}" for ema in emas])
    fastL = True
    for i in range (1, len (emas)):
        fastL = fastL & df [f"EMA_{emas [i-1]}"] > df [f"EMA_{emas [i]}"]
    fastS = True
    for i in range (1, len (emas)):
        fastL = fastL & df [f"EMA_{emas [i-1]}"] < df [f"EMA_{emas [i]}"]
    ...
    df["colFinal2"] = np.where((df["colslowS"] == True), "red", df["colFinal2"]) # Red = Sell
